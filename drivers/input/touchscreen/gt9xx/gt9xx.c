...
static void gtp_touch_down(struct goodix_ts_data* ts,s32 id,s32 x,s32 y,s32 w)
{
   ...
   GTP_INFO("++++++ID:%d, mGtpChange_X2Y:%d, mGtp_X_Reverse:%d, mGtp_Y_Reverse:%d", id, mGtpChange_X2Y, mGtp_X_Reverse, mGtp_Y_Reverse);
...
static void gtp_pen_init(struct goodix_ts_data *ts)
{
    ...
    input_set_abs_params(ts->pen_dev, ABS_MT_POSITION_X, 0, abs_y_max, 0, 0);
    input_set_abs_params(ts->pen_dev, ABS_MT_POSITION_Y, 0, abs_x_max, 0, 0);
...
#endif
    GTP_DEBUG("gtp_pen_down(%d)(%d, %d)[%d]\n", id, x, y, w);
}
...
static s32 gtp_get_info(struct goodix_ts_data *ts)
{
    ...
    GTP_INFO("gtp_get_info X_MAX = %d, Y_MAX = %d, TRIGGER = 0x%02x",
...
static s32 gtp_init_panel(struct goodix_ts_data *ts)
{
 ...
 #if GTP_PRINT_REGEDITS
     for (i = GTP_ADDR_LENGTH; i < ts->gtp_cfg_len; i++)
    {
       GTP_INFO(" config0 addr(%x, 0x%02X)", GTP_REG_CONFIG_DATA+i-GTP_ADDR_LENGTH, config[i]);
    }
#endif
    ...
    if (ret < 0)
    {
     ...
     for (i = GTP_ADDR_LENGTH; i < ts->gtp_cfg_len; i++)
    {
       GTP_INFO(" config1 addr(%x, 0x%02X)", GTP_REG_CONFIG_DATA+i-GTP_ADDR_LENGTH, config[i]);
    }
#endif
...
*/
    ...
    gtp_reset_guitar(ts->client, 20);
    GTP_INFO("   ts->irq  gtp_request_io_port \n");
    if(ret < 0)
    {
 ...
 //   GTP_DEBUG("INT trigger type:%x", ts->int_trigger_type);
    ...
    if (ts->irq)
    {
        ...
        ret = devm_request_threaded_irq(&(ts->client->dev), ts->irq, NULL, 
            goodix_ts_irq_handler,  initr,//IRQF_TRIGGER_RISING  IRQF_ONESHOT /*irq_table[ts->int_trigger_type]*/, ts->irq_flags |IRQF_TRIGGER_FALLING
            ts->client->name, ts);

	/*	if( ts->int_trigger_type)
			{
			   initr = IRQF_TRIGGER_FALLING;
			}
	 	ret=  request_irq(ts->irq, goodix_ts_irq_handler, initr, ts->client->name, ts);
	 	*/
	 
		 GTP_INFO("devm_request_threaded_irq:%d,%ld,%d\n",initr, ts->irq_flags, ret);
...
static int goodix_ts_early_resume(struct tp_device *tp_d)
{
    ...
    {
        hrtimer_start(&ts->timer, ktime_set(1, 0), HRTIMER_MODE_REL);
	 GTP_INFO("System create timer gtp.");
    }
 ...
 */
    ...
    if (ret < 0)
    {
        GTP_ERROR("<%s>_%d    I2C communication ERROR!\n", __func__, __LINE__);
